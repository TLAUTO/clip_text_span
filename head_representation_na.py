...
def main(args):
    ...
    # Load text:
    text_features = np.random.randn(3498, 768)
    ...
    with open(os.path.join(args.output_dir, f'{args.dataset}_completeness_{args.text_descriptions}_top_{args.texts_per_head}_heads_{args.model}.txt'), 'w') as w:
        re = []
        for i in tqdm.trange(attns.shape[1] - args.layers, attns.shape[1]):
            rein = []
    ...
    with open(os.path.join(args.output_dir, f'{args.dataset}_{args.texts_per_head}_navie_span_{args.layers}_{args.model}.npy'), 'wb') as f:
        np.save(f, re)

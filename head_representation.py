...
def replace_with_iterative_removal(data, text_features, texts, iters, rank, device):
    results = []
    text_representation = []
    nk = np.random.randn(795, 768)*1e-8
    data = data 
    ...
    for i in range(iters):
        ...
        data = data - ((data @ text_features[top_n]/text_norm)[:,np.newaxis] * text_features[
            top_n][np.newaxis, :])
        text_representation.append(text_features[top_n].tolist())
        text_features = text_features - (text_features @ text_features[top_n] / text_norm)[
            :,np.newaxis] * text_features[top_n][np.newaxis, :]
        text_features[top_n] = torch.zeros(768)
    return reconstruct, results, text_representation
...
def main(args):
    ...
    with open(os.path.join(args.output_dir, f'{args.dataset}_completeness_{args.text_descriptions}_top_{args.texts_per_head}_heads_{args.model}.txt'), 'w') as w:
        re = []
        for i in tqdm.trange( attns.shape[1]):
            rein = []
            for head in range(attns.shape[2]):
                _, _, reprersentation = replace_with_iterative_removal(attns[:, i, head], text_features, lines, args.texts_per_head, args.w_ov_rank, args.device)
                rein.append(reprersentation)
            re.append(rein)
        re = torch.tensor(re)
        re.cpu().numpy()
        re = np.array(re)
        print(isinstance(re, np.ndarray))
    with open(os.path.join(args.output_dir, f'{args.dataset}_{args.texts_per_head}span_{args.layers}_{args.model}_{args.lr}_Orthogonal.npy'), 'wb') as f:
    #with open(os.path.join(args.output_dir, f'{args.dataset}_{args.texts_per_head}span_{args.layers}_{args.model}_{args.lr}.npy'), 'wb') as f:
        np.save(f, re)

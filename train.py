...
set_seed(args.seed)
#with open(os.path.join(args.output_dir, f'{args.dataset}_da_{args.get_rate}_adamw_{args.lr}.txt'), 'w') as w:
with open(os.path.join(args.output_dir, f'{args.dataset}_da_{args.get_rate}_one_head_adamw_{args.lr}_128.txt'), 'w') as w: 
            ...
            for k in range(2):
                ...
                for epoch in range(args.epochs):  
                    ...
                    if current_acc > best[1]:
                        ...
                        best[1] = current_acc
                        if k:
                            #torch.save(model.state_dict(),args.model_path+args.dataset_name+'_'+f'{domain[n]}_to_{domain[m]}_one_head_adamw_8''.pt')
                            torch.save(model.state_dict(),args.model_path+args.dataset_name+'_'+f'{domain[n]}_to_{domain[m]}_adamw_128''.pt')
                        else:
                            #torch.save(model.state_dict(),args.model_path+args.dataset_name+'_'+f'{domain[m]}_to_{domain[n]}_one_head_adamw_8''.pt')
                            torch.save(model.state_dict(),args.model_path+args.dataset_name+'_'+f'{domain[m]}_to_{domain[n]}_adamw_128''.pt')
                    model.zero_grad()
    ...
    plt.ylabel('acc')
    plt.legend()
    plt.savefig(f'DA in {args.dataset}_mixdata_{args.get_rate}_one_head_adamw_{args.lr}_128.png')#
